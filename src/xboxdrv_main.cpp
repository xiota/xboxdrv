...
ControllerPtr XboxdrvMain::create_controller() {
  ...
  } else {  // regular USB Xbox360 controller
...
void XboxdrvMain::init_controller(const ControllerPtr& controller) {
  ...
  if (m_opts.rumble_l != -1 &&
      m_opts.rumble_r != -1) {  // Only set rumble when explicitly requested
  ...
  if (m_opts.instant_exit) {
    ...
    if (!m_opts.quiet) {
      ...
      if (m_opts.silent) {
        ...
        std::cout << "\nPress Ctrl-C to quit, use '--silent' to suppress the "
                     "event output"
                  << std::endl;
      }
    }
...
void XboxdrvMain::on_child_watch(GPid pid, gint status) {
...
void XboxdrvMain::print_info(libusb_device* dev, const XPadDevice& dev_type,
  ...
  std::cout << "Vendor/Product:    "
            << boost::format("%04x:%04x") % uint16_t(desc.idVendor) %
                   uint16_t(desc.idProduct)
            << std::endl;
  std::cout << "USB Path:          "
            << boost::format("%03d:%03d") %
  ...
  // ((opts.controller.back().uinput.force_feedback) ? "enabled" : "disabled")
  // << std::endl;
}

void XboxdrvMain::shutdown() {
