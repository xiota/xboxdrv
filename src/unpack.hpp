...
inline int16_t int16le(uint8_t* data) {
  if (is_big_endian()) {
    return swap16(*reinterpret_cast<int16_t*>(data));
  } else {
...
inline uint16_t uint16le(uint8_t* data) {
  if (is_big_endian()) {
    return swap16(*reinterpret_cast<uint16_t*>(data));
  } else {
...
inline int32_t int32le(uint8_t* data) {
  if (is_big_endian()) {
    return swap32(*reinterpret_cast<int32_t*>(data));
  } else {
...
inline uint32_t uint32le(uint8_t* data) {
  if (is_big_endian()) {
    return swap32(*reinterpret_cast<uint32_t*>(data));
  } else {
...
inline bool bit(uint8_t* data, int bit) { return (*data >> bit) & 1; }
