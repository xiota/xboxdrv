...
Options::GenericUSBSpec Options::GenericUSBSpec::from_string(
  ...
  process_name_value_string(
      str, boost::bind(&GenericUSBSpec::apply_pair, boost::ref(spec), _1, _2));
  return spec;
}
...
const ControllerOptions& Options::get_controller_options() const {
  ...
  if (it == controller_slots.end()) {
    ...
    ControllerSlotOptions::Options::const_iterator cfg =
        it->second.get_options().find(config_slot);
...
void Options::set_trigger_as_zaxis() {
  get_controller_options().uinput.trigger_as_zaxis();
}

void Options::set_dpad_as_button() {
...
void Options::set_mimic_xpad_wireless() {
  // BTN_BACK is recognized as mouse button, so we have to disallow
  // automatic allocation
  extra_devices = false;
  extra_events = false;
...
void Options::set_daemon() {
  mode = RUN_DAEMON;
  silent = true;
}

void Options::set_daemon_detach(bool value) {
  detach = value;
  if (detach) {
    silent = true;
    quiet = true;
...
void Options::set_match(const std::string& str) {
  process_name_value_string(str,
                            boost::bind(&Options::add_match, this, _1, _2));
}
...
void Options::set_match_group(const std::string& str) {
  ...
  process_name_value_string(
      str, boost::bind(&ControllerMatchRuleGroup::add_rule_from_string, group,
                       _1, _2));
...
Options::GenericUSBSpec Options::find_generic_usb_spec(int vendor_id_,
  ...
  for (std::vector<GenericUSBSpec>::const_iterator i =
    ...
    if (i->m_vendor_id == vendor_id_ && i->m_product_id == product_id_) {
      return *i;
    }
  }

  raise_exception(std::runtime_error, "no matching GenericUSBSpec found for "
                                          << boost::format("%04x:%04x") %
