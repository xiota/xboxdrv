  ...
  if (dev) {
    ...
    if (handle) {
      if (dev->descriptor.iProduct)
        usb_get_string_simple(handle, dev->descriptor.iProduct, iProduct_buf,
                              sizeof(iProduct_buf));
      usb_close(handle);
    }

    return out << boost::format(
                      "idVendor: 0x%04hx  idProduct: 0x%04hx  iProduct: %s") %
                      uint16_t(dev->descriptor.idVendor) %
                      uint16_t(dev->descriptor.idProduct) % iProduct_buf;
  } else {
...
void write_usb_device(struct usb_device* dev, int interface, int endpoint) {
  struct usb_dev_handle* handle = usb_open(dev);
  if (!handle) {
    std::cout << "Error opening usb device" << std::endl;
  } else {
    if (usb_claim_interface(handle, interface) != 0) {
      std::cout << "Error claiming the interface: " << usb_strerror()
                << std::endl;
      if (usb_detach_kernel_driver_np(handle, interface) < 0) {
        std::cout << "Failure to kick kernel driver: " << usb_strerror()
                  << std::endl;
        exit(EXIT_FAILURE);
      }
...
void read_usb_device(struct usb_device* dev, int interface, int endpoint) {
  struct usb_dev_handle* handle = usb_open(dev);
  if (!handle) {
    std::cout << "Error opening usb device" << std::endl;
  } else {
    ...
    while (!quit) {
      uint8_t data[8192];
      int ret =
          usb_interrupt_read(handle, endpoint, (char*)data, sizeof(data), 0);
      if (ret < 0) {
...
void cat_usb_device(struct usb_device* dev, int interface, int endpoint) {
  struct usb_dev_handle* handle = usb_open(dev);
  if (!handle) {
    std::cout << "Error opening usb device" << std::endl;
  } else {
    ...
    while (!quit) {
      ...
      if (0) {
        for (int i = 0; i < ret; ++i) data[i] = 255;
        ...
        for (int len = 3; len <= 8; ++len) {
          ...
          for (int front = 0; front < 256; ++front) {
            for (size_t i = 0; i < sizeof(arr); ++i) {
              char ledcmd[] = {front,  len,    arr[i], arr[i], arr[i], arr[i],
              ...
              uint8_t data[32];
              int ret = usb_interrupt_read(handle, endpoint, (char*)data,
                                           sizeof(data), 10);
              if (ret == -110) {
              } else if (ret < 0) {
                std::cout << "USBError: " << ret << "\n"
...
void list_usb_devices() {
  ...
  for (struct usb_bus* bus = busses; bus; bus = bus->next) {
    for (struct usb_device* dev = bus->devices; dev; dev = dev->next) {
      std::cout << boost::format("Bus %s Device %s ") % bus->dirname %
                       dev->filename
                << " " << dev << std::endl;
    }
    bus_idx += 1;
...
int main(int argc, char** argv) {
  ...
  if (argc == 2 && strcmp("list", argv[1]) == 0) {
    ...
    list_usb_devices();
             ...
             (strcmp("cat", argv[1]) == 0 || strcmp("read", argv[1]) == 0 ||
    ...
    if (sscanf(argv[2], "0x%hx", &idVendor) == 1 &&
      ...
      if (argc >= 5) interface = atoi(argv[4]);

      if (argc == 6) endpoint = atoi(argv[5]);
      ...
      if (!dev) {
        std::cout << "Error: Device ("
                  << boost::format("idVendor: 0x%04hx, idProduct: 0x%04hx") %
                         idVendor % idProduct
                  << ") not found" << std::endl;
      } else {
        if (strcmp("cat", argv[1]) == 0) {
          std::cout << "Reading data from: " << dev
                    << " Interface: " << interface << " Endpoint: " << endpoint
                    << std::endl;
    ...
    std::cout << "Usage: " << argv[0] << " list\n"
              << "       " << argv[0]
              << " cat IDVENDOR IDPRODUCT [INTERFACE] [ENDPOINT]" << std::endl;
  }
}
