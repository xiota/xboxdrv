    ...
    if (handle) {
      if (dev->descriptor.iProduct) {
        usb_get_string_simple(
            handle, dev->descriptor.iProduct, iProduct_buf, sizeof(iProduct_buf)
        );
      }
      usb_close(handle);
    }
        ...
        for (int len = 3; len <= 8; ++len) {
          ...
          for (int front = 0; front < 256; ++front) {
            for (size_t i = 0; i < sizeof(arr); ++i) {
              char ledcmd[] = {
                front,  len,    arr[i], arr[i], arr[i], arr[i], arr[i], arr[i],
...
void list_usb_devices() {
  ...
  for (struct usb_bus *bus = busses; bus; bus = bus->next) {
    for (struct usb_device *dev = bus->devices; dev; dev = dev->next) {
      std::cout << std::format("Bus {:s} Device {:s}  {:s}", bus->dirname, dev->filename, dev)
                << std::endl;
    }
    bus_idx += 1;
  }
...
int main(int argc, char **argv) {
  ...
  if (argc == 2 && strcmp("list", argv[1]) == 0) {
    ...
    if (sscanf(argv[2], "0x%hx", &idVendor) == 1 && sscanf(argv[3], "0x%hx", &idProduct) == 1) {
      if (argc >= 5) {
      ...
      if (!dev) {
        ...
        if (strcmp("cat", argv[1]) == 0) {
          std::cout << "Reading data from: " << dev
                    << " Interface: " << interface << " Endpoint: " << endpoint << std::endl;
