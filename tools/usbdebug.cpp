...
class EndpointListenerThread {
 ...
 public:
  EndpointListenerThread(int endpoint_) : endpoint(endpoint_), running(false) {}
...
class Sequence {
 ...
 public:
  Sequence(int start_, int end_) : start(start_), end(end_), idx(start_) {}

  Sequence(int start_) : start(start_), end(start_), idx(start_) {}
...
void dispatch_command(const std::vector<std::string> &args) {
  ...
  if (args[0] == "help") {
    ...
    std::cout << "Unknown command '" << args[0]
              << "', type 'help' to list all available commands" << std::endl;
  }
}
